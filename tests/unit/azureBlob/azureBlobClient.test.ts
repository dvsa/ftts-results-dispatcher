  ...
  describe('uploadFile', () => {
    ...
    test('GIVEN upload fails WHEN called THEN the AzureBlobError is thrown', async () => {
      ...
      try {
        ...
        expect(error.message).toEqual('Failed to upload a file with error code: 400');
        expect(error.cause).toBeUndefined();
        expect(error.properties).toEqual({ fileName: FILE_NAME, containerName: CONTAINER_NAME });
      }
    });
  });
