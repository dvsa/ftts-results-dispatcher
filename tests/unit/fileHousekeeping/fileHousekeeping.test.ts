...
describe('fileHousekepping', () => {
  ...
  test('GIVEN 2 processed file names and 2 metadata file names WHEN call fileHousekeeping function THEN old files succesfully deleted', async () => {
    ...
    when(mockedAzureBlobClient.listFiles)
      ...
      .mockResolvedValue([METADATA_FILE_NAME_1, METADATA_FILE_NAME_LAST]);
    ...
    expect(mockedLogger.info).toHaveBeenNthCalledWith(
      1,
      'Trying to delete old processed files',
      { shareName: SHARE_NAME },
    );
    expect(mockedLogger.info).toHaveBeenNthCalledWith(
      2,
      `${METADATA_FILE_NAME_1} deleted by housekeeping cron job`,
      { containerName: CONTAINER_NAME },
    );
    expect(mockedLogger.info).toHaveBeenNthCalledWith(
      3,
      `The old metadata files ${JSON.stringify([METADATA_FILE_NAME_1])} deleted from the Azure Blob Container`,
      { containerName: CONTAINER_NAME },
    );
  ...
  test('GIVEN 3 unsorted metadata file names WHEN call fileHousekeeping function THEN old files succesfully deleted', async () => {
    when(mockedAzureBlobClient.listFiles)
      .calledWith(CONTAINER_NAME)
      .mockResolvedValue([METADATA_FILE_NAME_2, METADATA_FILE_NAME_LAST, METADATA_FILE_NAME_1]);
    ...
    expect(mockedLogger.info).toHaveBeenNthCalledWith(
      2,
      `${METADATA_FILE_NAME_2} deleted by housekeeping cron job`,
      { containerName: CONTAINER_NAME },
    );
    expect(mockedLogger.info).toHaveBeenNthCalledWith(
      3,
    ...
    expect(mockedLogger.info).toHaveBeenNthCalledWith(
      4,
      `The old metadata files ${JSON.stringify([METADATA_FILE_NAME_2, METADATA_FILE_NAME_1])} deleted from the Azure Blob Container`,
      { containerName: CONTAINER_NAME },
    );
    expect(mockedLogger.logEvent).toHaveBeenCalledTimes(1);
    expect(mockedLogger.logEvent).toHaveBeenCalledWith(
      BusinessTelemetryEvent.RES_TARS_RESULTS_FILES_DELETED,
      'Azure File Storage Share is empty',
      {
        shareName: SHARE_NAME,
        deletedFiles: '',
      },
