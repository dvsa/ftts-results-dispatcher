...
import { BasicMetadata } from '../../../src/metadata/metadata';
  ...
  describe('createFile', () => {
    ...
    test('GIVEN test results AND metadata WHEN called AND fails THEN an error is thrown ', () => {
      ...
      try {
        ...
        expect(error).toEqual(new FileGenerationError(`Failed to create a XML test results file. ${errorMessage}`));
      }
    });
    ...
    test('GIVEN test results AND metadata WHEN called THEN an xml file is returned', () => {
      ...
      expect(mockedLogger.info).toHaveBeenNthCalledWith(
        1,
        'Starting create a XML test results file',
      );
      expect(mockedLogger.info).toHaveBeenNthCalledWith(
        2,
        'Successfully created a XML test results file',
      );
