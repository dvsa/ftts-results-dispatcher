...
const azureFilesClient = new AzureFilesClient(mockedShareServiceClient);
...
describe('AzureFilesClient', () => {
  ...
  describe('newAzureFilesClient', () => {
    test('GIVEN config.azureStorage.azureFiles.storageConnectionString is defined WHEN called THEN azureFilesClient is created using the connection string', () => {
      mockedConfig.azureFiles.storageConnectionString = 'BlobEndpoint=https://dsuksanresapistor001.blob.core.windows.net...';
      ...
      expect(ShareServiceClient.fromConnectionString).toHaveBeenCalledWith(
        mockedConfig.azureFiles.storageConnectionString,
      );
  ...
  describe('uploadFile', () => {
    ...
    test('GIVEN no errors WHEN called THEN the file is successfully uploaded', async () => {
      ...
      when(mockedShareFileClient.uploadRange)
        .calledWith(CONTENT, 0, CONTENT.length)
        .mockResolvedValue({
    ...
    test('GIVEN an error on file create WHEN called THEN re-throws the error', async () => {
      ...
      try {
        ...
        expect(error.message).toEqual(`Failed to create an empty file: ${ERR_CODE}`);
        expect(error.cause).toBeUndefined();
        expect(error.properties).toEqual({ shareName: SHARE_NAME, fileName: FILE_NAME });
      }
    });
    ...
    test('GIVEN an error on file upload WHEN called THEN re-throws the error', async () => {
      ...
      try {
        ...
        expect(error).toBeInstanceOf(AzureFilesError);
        expect(error.message).toEqual(`Failed to upload the contents of a file: ${ERR_CODE}`);
        expect(error.cause).toBeUndefined();
        expect(error.properties).toEqual({ shareName: SHARE_NAME, fileName: FILE_NAME });
      }
    });
  ...
  describe('deleteFile', () => {
    ...
    test('GIVEN an error on file delete WHEN call deleteFile THEN re-throws the error', async () => {
      ...
      await expect(azureFilesClient.deleteFile(
        SHARE_NAME,
        FILE_NAME,
      )).rejects.toEqual(new AzureFilesError(`Failed to delete the file. Error code: ${ERR_CODE}`));
      expect(mockedShareDirectoryClient.deleteFile).toHaveBeenCalledTimes(1);
    });
  });
});
